...
The separate `/pdc` command has two deliberately different pipelines. `/pdc today` and `/pdc tomorrow` establish official tournaments and matchups from PDPA event pages, then reconcile them with the current Darts Nerd fixture feed so late withdrawals, replacements, and match times supersede stale draw text only when a live row shares an official participant. Every unique player is resolved through DartsOrakel and receives their latest 10 completed matches with result, opponent, score, date, three-dart average, 180 count, and checkout percentage (the validated history model also retains tournament, round, and checkout hit/attempt evidence). The three statistics views are paced below the public Reader transport's rolling allowance and run as background webhook work: Telegram acknowledges the scan immediately, then sends the completed cards. A metric that DartsOrakel genuinely does not publish for an individual match remains undefined and is displayed as unavailable without failing that player or report. `/pdc latest` independently scans the DartsOrakel PDC calendar and retrieves the most recently completed tournament result page. PDC never uses the MODUS source or report endpoint.

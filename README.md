...
Send the bot a private message such as `Dylan Slevin last 10 match`, `Jack Drayton last 10 matches from MODUS`, or `Rob Cross last 10 matches from DartsOrakel`. “Average,” “stats,” and plural “matches” are optional. DartsOrakel responses include each match's average, 180 count, and checkout percentage, plus mean/best average, total 180s, weighted checkout conversion with hit/attempt evidence, metric coverage, and the win/loss/draw record. Without `from ...`, source selection is automatic: players in the bundled current MODUS series use official MODUS match details, while every other player goes immediately to DartsOrakel without a MODUS network request. MODUS currently publishes match averages through this integration; 180 and checkout fields remain visibly unavailable instead of being mixed from another source. An explicit `from MODUS` request checks the live official catalogue even when the bundled index has not yet seen today's player. MODUS rows include their official proof URL. `/start` and `/help` show examples; `/health` verifies Telegram delivery without calling a statistics source. Updates from every other user, group, or channel are silently ignored.

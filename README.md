...
The separate `/pdc` command has two deliberately different pipelines. `/pdc today` and `/pdc tomorrow` establish official tournaments and matchups from PDPA event pages, then reconcile them with the current Darts Nerd fixture feed so late withdrawals, replacements, and match times supersede stale draw text only when a live row shares an official participant. Every unique player is resolved through DartsOrakel and receives their latest 10 completed matches with result, opponent, score, date, and three-dart average (the validated history model also retains tournament and round). Optional 180/checkout views are not multiplied across the whole field, so those values are shown as unavailable rather than triggering public-reader throttling. `/pdc latest` independently scans the DartsOrakel PDC calendar and retrieves the most recently completed tournament result page. PDC never uses the MODUS source or report endpoint.
